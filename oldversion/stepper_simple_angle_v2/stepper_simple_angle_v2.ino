/*
  stepper_simple_angle_v2.ino
  ───────────────
  28BYJ-48 ステッピングモーターを使った
  シンプルな角度制御プログラム（ログ機能付き）
  
  作成者: Koki Takagi
  作成日: 2025年5月10日
  更新日: 2025年5月10日 - ログ機能追加
*/
...
// シリアル通信のCSVヘッダー
const String CSV_HEADER = "timestamp,angle";
...
void setup() {
  ...
  // CSVヘッダーを送信（PCでログファイル作成時に使用）
  Serial.println("LOG:" + CSV_HEADER);
  
  // 使用方法の表示
  Serial.println("28BYJ-48 シンプル角度制御 v2（ログ機能付き）");
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    // 特別コマンドの処理
      ...
      Serial.println("0-360の角度、または r (リセット)、p (位置表示)、t (時間表示) を入力してください");
    }
  }
}
...
void moveToAngle(int angle) {
  ...
  // 時刻と角度をログとして送信
  logAngleChange(angle);
}

// 時刻と設定角度をログとしてシリアルに送信する関数
void logAngleChange(int angle) {
  // 起動からの経過時間（ミリ秒）をタイムスタンプとして使用
  unsigned long timestamp = millis();
  
  // CSV形式でログデータをシリアルに送信
  // LOG:プレフィックスをつけることで、PCソフトウェアがログデータとして識別できるようにする
  Serial.print("LOG:");
